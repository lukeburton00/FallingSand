...
public:
    ...
    Element* getElementToActOn()
    {
        ...
        if (world->inBounds(x, y - 1) && world->getElementAtPosition(x, y - 1)->type != ElementType::STEAM)
        {
            element = world->getElementAtPosition(x, y - 1);
        }

        else if (world->inBounds(x - 1, y) && world->getElementAtPosition(x - 1, y)->type != ElementType::STEAM)
        {
        ...
        else if (world->inBounds(x + 1, y - 1) && world->getElementAtPosition(x + 1, y - 1)->type != ElementType::STEAM)
        {
